...
def foundation_bed(name,top_z=.3972,bottom_z=-.18,top_size=(120.0,90.0),bottom_size=(132.0,102.0),mat=None):
    ...
    if len(top2)!=len(toe2): raise RuntimeError("foundation ring topology mismatch")
    n=len(top2)
    ...
    for i in range(n):
        j=(i+1)%n; faces.append((n+i,n+j,2*n+j,2*n+i))
    ...
    for i in range(n):
        j=(i+1)%n; x0,y0=top2[i]; x1,y1=top2[j]
        total += math.hypot(x1-x0,y1-y0); lengths.append(total)
    total=max(total,1e-6); ring_u=[lengths[i]/total for i in range(n)]
    for poly in mesh.polygons:
        for li in poly.loop_indices:
            vi=mesh.loops[li].vertex_index; ring=vi//n; idx=vi%n
            uv.data[li].uv=(ring_u[idx],1.0 if ring==0 else (0.25 if ring==1 else 0.0))
        if poly.index<n: poly.use_smooth=True
...
def bolt_ring(prefix,center,radius,z,count,mat,bolt_r=.035,bolt_h=.045):
    for i in range(count):
        a=2*math.pi*i/count
        cyl(f"{prefix}_{i}",(center[0]+math.cos(a)*radius,center[1]+math.sin(a)*radius,z),bolt_r,bolt_h,mat,8)

def insulator_stack(name,loc,height,M,detail=2,brown=False):
    mat=M["brown_porcelain"] if brown else M["porcelain"]
    if brown:
        core_r=.115; shed_r=.305 if height>=2.6 else .275; full_sheds=max(5,min(10,int(round(height/.40))))
    else:
        core_r=.090 if height>=2.8 else .105; shed_r=.255 if height>=2.8 else .235; full_sheds=max(6,min(13,int(round(height/.29))))
    if detail>=2: sheds=full_sheds; seg=24
    elif detail==1: sheds=max(5,int(round(full_sheds*.72))); seg=16
    else: sheds=max(3,int(round(full_sheds*.46))); seg=10
    cap_h=min(.16,max(.10,height*.045))
    z_bottom=loc[2]-height/2; z_top=loc[2]+height/2
    ceramic_bottom=z_bottom+cap_h; ceramic_top=z_top-cap_h; ceramic_h=max(.25,ceramic_top-ceramic_bottom); step=ceramic_h/sheds
    profile=[(ceramic_bottom,core_r*1.24),(ceramic_bottom+step*.10,core_r*1.18),(ceramic_bottom+step*.15,core_r)]
    for i in range(sheds):
        base=ceramic_bottom+i*step; alt=1.0 if (brown or i%2==0) else .92; sr=shed_r*alt
        profile.extend([
            (base+step*.16,core_r),(base+step*.23,core_r*1.10),(base+step*.31,sr*.58),
            (base+step*.39,sr*.90),(base+step*.44,sr),(base+step*.49,sr*.98),
            (base+step*.55,sr*.67),(base+step*.61,core_r*1.22),(base+step*.72,core_r),(base+step*.92,core_r),
        ])
    profile.extend([(ceramic_top-step*.08,core_r),(ceramic_top-step*.03,core_r*1.16),(ceramic_top,core_r*1.24)])
    clean=[]; last_z=-1e30
    for z,r in profile:
        z=max(z,last_z+1e-5); clean.append((z,r)); last_z=z
    profile=clean
    verts=[]
    for z,r in profile:
        for j in range(seg):
            a=2*math.pi*j/seg; verts.append((loc[0]+r*math.cos(a),loc[1]+r*math.sin(a),z))
    faces=[]; rings=len(profile)
    for ri in range(rings-1):
        for j in range(seg):
            nj=(j+1)%seg; faces.append((ri*seg+j,ri*seg+nj,(ri+1)*seg+nj,(ri+1)*seg+j))
    verts.append((loc[0],loc[1],profile[0][0])); bot=len(verts)-1
    verts.append((loc[0],loc[1],profile[-1][0])); top=len(verts)-1
    for j in range(seg):
        nj=(j+1)%seg; faces.append((bot,j,nj)); a0=(rings-1)*seg+j; a1=(rings-1)*seg+nj; faces.append((top,a1,a0))
    mesh=bpy.data.meshes.new(name+"_mesh"); mesh.from_pydata(verts,[],faces); mesh.update()
    uv=mesh.uv_layers.new(name="UVMap"); zmin=profile[0][0]; zmax=profile[-1][0]; zr=max(.001,zmax-zmin)
    for poly in mesh.polygons:
        for li in poly.loop_indices:
            vi=mesh.loops[li].vertex_index; vx,vy,vz=mesh.vertices[vi].co; dx=vx-loc[0]; dy=vy-loc[1]
            u=(math.atan2(dy,dx)/(2*math.pi))%1.0 if abs(dx)+abs(dy)>1e-8 else .5; v=max(0.0,min(1.0,(vz-zmin)/zr)); uv.data[li].uv=(u,v)
        poly.use_smooth=True
    o=bpy.data.objects.new(name,mesh); bpy.context.collection.objects.link(o); o.data.materials.append(mat)
    flange_r=max(core_r*1.75,shed_r*.56); neck_r=core_r*1.28; verts_hw=20 if detail>=2 else (14 if detail==1 else 10)
...
def lattice_post(name,x,y,z0,h,M,detail=2,width=.72):
    for sx in (-1,1):
        for sy in (-1,1): box(f"{name}_leg_{sx}_{sy}",(x+sx*width/2,y+sy*width/2,z0+h/2),(.065,.065,h),M["galv"],.008)
    if detail>=1:
        steps=max(2,int(h/1.4))
        for j in range(steps+1):
            z=z0+h*j/steps; box(f"{name}_ringx_{j}",(x,y,z),(width+.06,.045,.045),M["galv"],.006); box(f"{name}_ringy_{j}",(x,y,z),(.045,width+.06,.045),M["galv"],.006)
        if detail>=2:
            for j in range(steps):
                z=z0+h*(j+.5)/steps; ang=math.radians(35)
...
def fan_guard(name,loc,r,M,detail=2,rot=(math.radians(90),0,0)):
    ...
    if detail>=2:
        for i in range(8):
            a=2*math.pi*i/8; box(f"{name}_spoke_{i}",loc,(r*1.55,.025,.022),M["steel"],.003,rot=(0,-a,0))
        for i in range(5):
            rr=r*(i+1)/6; torus(f"{name}_mesh_{i}",loc,rr,.008,M["steel"],rot=rot,major_segments=20,minor_segments=4)
